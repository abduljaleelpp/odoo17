...
export class ActiveDealsCount extends Component {
    static template = "awesome_dashboard.ActiveDealsCount";
    static components = { Layout, DashboardItem, FilterPanel };

    setup() {
        this.statisticsService = useState(useService("awesome_dashboard.active_deals_statistics"));
        this.filterOptionsService = useService("filter_options_service");
        this.items = registry.category("active_deals_count_items").getAll();
