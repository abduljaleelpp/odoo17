...
export class SalesDemographics extends Component {
    static template = "awesome_dashboard.SalesDemographics";
    static components = { Layout, DashboardItem, FilterPanel };

    setup() {
        this.statisticsService = useState(useService("awesome_dashboard.sales_demographics_statistics"));
        this.filterOptionsService = useService("filter_options_service");
        this.items = registry.category("sales_demographics_items").getAll();
...
registry.category("lazy_components").add("SalesDemographics", SalesDemographics);