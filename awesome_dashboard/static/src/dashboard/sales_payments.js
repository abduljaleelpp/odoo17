...
export class SalesPayments extends Component {
    static template = "awesome_dashboard.SalesPayments";
    static components = { Layout, DashboardItem, FilterPanel };

    setup() {
        this.statisticsService = useState(useService("awesome_dashboard.sales_payments_statistics"));
        this.filterOptionsService = useService("filter_options_service");
        this.items = registry.category("sales_payments_items").getAll();
...
registry.category("lazy_components").add("SalesPayments", SalesPayments);